...
const CardCategoria = ({ id, areaId, nombre, descripcion, color, canDelete, onDelete }) => {
    ...
    >
        ...
        <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-2">
              ...
              />
              <p className="inventory-card-meta text-xs uppercase tracking-wide text-slate-500">
                Categoría
              </p>
            </div>
            <h3 className="inventory-card-title font-semibold text-slate-900 truncate">{nombre}</h3>
            <p className="inventory-card-desc text-sm text-slate-600 mt-1 line-clamp-2 min-h-[40px]">
              {descripcion || "Sin descripción"}
            </p>
            <div className="inventory-card-link mt-3 inline-flex items-center gap-1 text-sm text-blue-700 group-hover:text-blue-800">
              Ver productos <RightOutlined className="text-xs" />
            </div>
        </div>
